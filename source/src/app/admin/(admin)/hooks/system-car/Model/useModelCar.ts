...
const fetchModelCarList = async (brandId:string|null,searchParams: any, page: number): Promise<any> => {
    const response = await fetch(`/api/admin/car-model/${brandId}?${searchParams}&page=${page}`);
    if (!response.ok) {
    ...
    useEffect(() => {
        if (!isPlaceholderData && page < modelCarList?.totalPage) {
            queryClient.prefetchQuery({
                queryKey: [QUERY_KEY.modelCar,brandId, searchParams.toString(), page + 1],
                queryFn: () => fetchModelCarList(brandId,searchParams.toString(), page + 1),
            });

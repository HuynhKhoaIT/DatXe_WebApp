...
const fetchYearCarList = async (modelId:string|null,searchParams: any, page: number): Promise<any> => {
    const response = await fetch(`/api/admin/car-model/${modelId}?${searchParams}&page=${page}`);
    if (!response.ok) {
...
export const useYearCar = (modelId:string|null): UseNews => {
    ...
    useEffect(() => {
        if (!isPlaceholderData && page < yearCarList?.totalPage) {
            queryClient.prefetchQuery({
                queryKey: [QUERY_KEY.yearCar,modelId, searchParams.toString(), page + 1],
                queryFn: () => fetchYearCarList(modelId,searchParams.toString(), page + 1),
            });
    ...
    const { mutate: deleteItem } = useMutation({
        ...
        onSuccess: () => {
            notifications.show({
                title: 'Thành công',
                message: 'Xoá năm sản xuất thành công',
            });

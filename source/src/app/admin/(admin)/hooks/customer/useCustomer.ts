...
const fetchCustomers = async (
  ...
  const response = await fetch(
    `/api/admin/customer?${searchParams}&page=${page}`
  );
  if (!response.ok) {
...
const fetchCustomersDlbd = async (
  searchParams: any,
  page: number
): Promise<any> => {
  const response = await fetch(
    `/api/admin/customer/dlbd?${searchParams}&page=${page}`
  );
  if (!response.ok) {
    throw new ResponseError("Failed to fetch customers", response);
  }
...
export const useCustomers = (): useCustomers => {
  ...
  useEffect(() => {
    if (
      ...
      queryClient.prefetchQuery({
        queryKey: [
          QUERY_KEY.customersDlbd,
          searchParams.toString(),
          pageDlbd + 1,
        ],
        queryFn: () =>
          fetchCustomersDlbd(searchParams.toString(), pageDlbd + 1),
      });
    } else if (
      activeTab == "second" &&
